...
/// </summary>
[ExecuteAlways]
public class StableDiffusionText2MaterialControlNet : StableDiffusionGenerator
{
    ...
    [Range(1, 100)] public int tilingX = 1;
    ...
    {
        Debug.Log($"IS generating {generating} | {prompt}");
    ...
    IEnumerator GenerateAsync()
    {
        ...
        CaptureDepthTexture.RequestDepthTexture(width, height, tex => StartCoroutine(SubmitFromDepthTexture(tex)));
    }
    ...
    IEnumerator SubmitFromDepthTexture(Texture2D depthTexture)
    {
        ...
        try
        {
            ...
            {
                SDParamsInTxt2ImgControlNet sd = new SDParamsInTxt2ImgControlNet();
                sd.prompt = prompt;
    ...
    void LoadIntoMaterial(Texture2D texture)
    {
        try
        {
            ...
            Shader standardShader = sdc.settings.useUniversalRenderPipeline
                ? Shader.Find("Universal Render Pipeline/Lit")
                : Shader.Find("Standard");

            if (!standardShader)
                Debug.LogError(
                    "Shader setup wrong: Please check if you're project uses 'Standard' or 'Universal Render Pipeline'");
